...
export default function Home() {
  ...
  const sendQuery = async (userMessage: string) => {
    ...
    try {
      ...
      if (schedHeader) {
        const action = JSON.parse(decodeURIComponent(schedHeader)) as SchedulingAction;
        setScheduling({
          action,
    ...
    <div className='flex flex-col h-screen bg-copilot-bg'>
      ...
      <header className='flex items-center justify-between px-4 py-3 border-b border-copilot-border bg-copilot-sidebar'>
        <div className='flex items-center gap-3'>
          ...
          <h1 className='text-lg font-semibold text-white'>Medical Records Assistant</h1>
        </div>
      </header>
        ...
        {messages.length === 0 ?
          <div className='flex flex-col items-center justify-center h-full text-copilot-muted'>
            ...
            <p className='text-sm text-center max-w-lg'>
              Patient data is split between structured fields (diagnoses, medications, labs) and
              free-text clinical notes. This assistant searches{' '}
              <strong className='text-copilot-text'>both</strong> — exact counts and filters from
              the database, meaning-based search over the notes — and answers in plain English,
              grounded in the actual records. You can also schedule appointments.
            </p>
              ...
              <div className='flex justify-start'>
                <div className='max-w-[80%] rounded-lg px-4 py-4 bg-copilot-sidebar border border-copilot-accent'>
                  <div className='flex items-center gap-2 mb-3'>
                    ...
                    <span className='font-semibold text-copilot-text'>Schedule Appointment</span>
                  </div>

                  <p className='text-sm text-copilot-muted mb-3'>
                    Patient:{' '}
                    <strong className='text-copilot-text'>{scheduling.action.patientName}</strong>
                  </p>

                  <div className='grid grid-cols-2 gap-3 mb-4'>
                    <div>
                      <label className='block text-xs text-copilot-muted mb-1'>Date</label>
                      <input
                        type='date'
                        value={scheduling.date}
                        onChange={e =>
                          setScheduling(prev => (prev ? { ...prev, date: e.target.value } : null))
                        }
                        className='w-full px-3 py-2 bg-copilot-input border border-copilot-border rounded text-copilot-text text-sm'
                      />
                    </div>
                    <div>
                      <label className='block text-xs text-copilot-muted mb-1'>Time</label>
                      <input
                        type='time'
                        value={scheduling.time}
                        onChange={e =>
                          setScheduling(prev => (prev ? { ...prev, time: e.target.value } : null))
                        }
                        className='w-full px-3 py-2 bg-copilot-input border border-copilot-border rounded text-copilot-text text-sm'
                      />
                    </div>
                  ...
                  {scheduling.result && !scheduling.result.success && (
                    <p className='text-sm text-red-400 mb-3'>{scheduling.result.message}</p>
                  )}
